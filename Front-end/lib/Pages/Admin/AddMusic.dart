...
class AddMusic extends StatefulWidget {
  ...
  const AddMusic({super.key, this.uid, this.portfolio, this.fullName, this.province});
...
class _AddMusicState extends State<AddMusic> {
  ...
  Future<void> uploadSong() async {
    ...
    try {
      ...
      if (isVideo) {
        ...
        } else if (_selectedFile is PlatformFile &&
            (_selectedFile as PlatformFile).bytes != null) {
  ...
  Future<Uint8List> _convertToAudioBytes(String originalFileName) async {
    ...
    if (_selectedFile is io.File) {
      ...
      request.files.add(
        http.MultipartFile.fromBytes(
          'video_file',
          (_selectedFile as PlatformFile).bytes!,
    ...
    try {
      ...
      throw Exception(
        'Conversion timed out. The file might be too large or the server is slow.',
      );
    } catch (e) {
                      ...
                      return Theme(
                        data: color.copyWith(
                          colorScheme: ColorScheme.light(
                            ...
                            onPrimary:
                                color.scaffoldBackgroundColor, // Header text
                            onSurface:
                                color.textTheme.bodyLarge?.color ??
                                Colors.black, // Calendar text
                          ),
                        ),
                        child: child!,
                      );
