...
class _UniversityApplicationScreenState
  ...
  bool _isFirstApplication = true; // NEW: Track if it's the user's first application
  ...
  Future<void> _checkCurrentUserAndApplications() async {
    ...
    if (_currentUser != null) {
      ...
      // UPDATED: Check for the isFirstApplication flag
      if (userDoc.exists &&
  ...
  Future<void> _submitApplication() async {
    ...
    try {
      ...
      String? idPassportUrl = _idPassportUrl ??
          await _uploadFile(
            _idPassportFile!,
            '${storagePathPrefix}id_passport.pdf',
          );
      String? schoolResultsUrl = _schoolResultsUrl ??
          await _uploadFile(
            _schoolResultsFile!,
            '${storagePathPrefix}school_results.pdf',
          );
      String? passportPhotoUrl = _passportPhotoUrl ??
          await _uploadFile(
            _passportPhotoFile!,
      ...
      String? finalThirdChoiceProgram = _thirdChoiceProgram == 'Other' // New final program
      ...
      DocumentReference globalApplicationRequestRef =
          await _firestore.collection('application_requests').add({
        'userId': _currentUser!.uid,
  ...
  // UPDATED: _buildCupertinoTextField
  Widget _buildCupertinoTextField({
    ...
    bool readOnly = false, // This is now a parameter
    int? maxLines = 1,
  }) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0),
  ...
  // UPDATED: _buildFilePickerTile to allow re-picking
  Widget _buildFilePickerTile({
    ...
    final String subtitleText = hasFile ? 'File uploaded' : 'No file selected';
    ...
    return Card(
      ...
      child: CupertinoListTile(
        ...
        onTap: () async {
          // Allow picking even if a file exists, to replace it
          await onPick(null);
        },
      ),
    );
  ...
  // UPDATED: _buildMultipleFilePickerTile to allow re-picking
  Widget _buildMultipleFilePickerTile({
    ...
    return Card(
      ...
      child: CupertinoListTile(
        ...
        onTap: () async {
          // Allow picking again to replace files
          await onPick([]);
        },
      ),
    );
  ...
  Widget build(BuildContext context) {
    final color = Theme.of(context);
    // Use the new flag to control form editability
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildCupertinoTextField(
                              ...
                              keyboardType: TextInputType.emailAddress,
                              readOnly: true, // This should always be read-only
                            ),
                            _buildCupertinoTextField(
                                ...
                                if (_thirdChoiceProgram == 'Other')
                                  _buildCupertinoTextField(
                                    controller:
                                        _otherThirdChoiceProgramController,
                                    placeholder:
                                        'Enter Third Choice Course Name',
                                    prefixIcon: CupertinoIcons.text_cursor,
                                  ),
                              ],
